...
- ### General
	...
	- Weapon switch/deploy speed is now 0.5 seconds (from 0.66 seconds). Animation speed has not been changed.
...
- ### General
	...
	- #### Arena/Payload Race
		- Removed next level change logic after a certain amount of rounds have been played

- ### 2Fort
	- Updated the color correction
	- Updated the sewer entrance layout
	
- ## Dustbowl
	- Fixed being able to capture the point below the first Stage 2 capture point
...
- ### Well (Arena)
	...
	- Fixed multiple props being misaligned
...
- ## Chariots
	- Added a dropdown or some shit idk (TEMPORARY NOTE CHANGE THIS)
	-
...
- Vote kicks will now immediately pass if the player being vote kicked leaves the game
...
- The Flamethrower's secondary fire charging mechanism now has proper client prediction
...
- Fixed a bug where attacking disguised Spies on the Payload Cart/Dispenser would show their own team's healing particle
...
- Fixed a bug where voice commands were turned off by default in the chat window, despite the chat window showing otherwise
