...
- Added ability to disarm grenades by striking them with a melee weapon
  ...
  - You can disarm the caltrop grenade by shooting it as well 
...
- Weapons now have crit glow and particle (from TF2Classic)
...
- Nailgun now deals less damage vs. sentries at higher levels
...
- Added Crossover (KOTH)
  - Port of the TFC map, has bridge like 2Fort.

## HUD

- Added a brand new main menu (Old menu still available via the `-oldui` launch option.)
- Added a loadout menu (currently no weapons)
...
- Certain HUD elements are now excluded from `cl_drawhud` (from Open Fortress)
...
- Sniper rifle fully charged bell
- Medics now receive the damage events dealt by their healing target

## Source Mod Plugins
- Source Mod is now supported using [PF2-Tools](https://github.com/dangreene0/PF2-Tools).
- Special thanks to **Conneath**.
- All base TF2 plugins should function.
